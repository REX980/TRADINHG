document.addEventListener('DOMContentLoaded', function () {
  ...
  function updateBalanceDisplay() {
    ...
    // Also update card balance if present
  ...
  function initializeDemo() {
    ...
    const cardBalance = document.querySelector('.card p');
    if (cardBalance) cardBalance.textContent = `$${displayBalance.toFixed(2)}`;
    const walletBalance = document.querySelector('.balance-amount');
    if (walletBalance) walletBalance.textContent = `$${displayBalance.toFixed(2)}`;
